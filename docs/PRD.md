...
| **M2 = MVP** | StableNet core parity | + **SNMP v2c identity probing** *(v0: sysName/sysDescr/sysUpTime → hostname/vendor/OS hints; GETBULK ifTable interface inventory with GETNEXT fallback)*; LLDP/CDP topology; config backup+diff (SSH) *(v0: opt-in dedicated `inspect` enrichment, strict known-hosts/key-reference transport, encrypted FR-CFG-001 vault references, Cisco IOS-XE + Aruba AOS-CX profiles, raw/normalized snapshots)*; scheduled reports PDF *(partial: stored daily HTML/PDF with explicit renderer; cron delivery remains)*; triage wizard; scale test passing AC-NFR-01/02 *(deterministic synthetic scheduler/progress and deadline-bounded down-confirm→alarm enqueue seams added; release evidence tracked in docs/TASKS.md)* | Discovery/Core NMS alpha → Enterprise beta |
| **M3** | NetBrain context | Graph-backed topology (temporal/provenance model §6); L2 path computation; diagnostic bundles/runbooks (read-only); **intents v0 + violation alarms ✅**; map time-travel | Diagnostics & digital twin |
