...
| **M2 = MVP** | StableNet core parity | + **SNMP v2c identity probing ?** *(v0: sysName/sysDescr/sysUpTime ? hostname/vendor/OS hints; GETBULK/ifTable next)* & interface inventory; LLDP/CDP topology; config backup+diff (SSH); scheduled reports PDF; triage wizard; scale test passing AC-NFR-01/02 | Discovery/Core NMS alpha → Enterprise beta |
