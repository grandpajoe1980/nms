...
| **M1** | Hardening | Split crates per §11; OpenAPI spec ✅ *(v0 served at `/api/openapi.json`)*; auth modes; spool-on-disk collectors; health endpoint ✅ *(`/api/health`)*; fixture-based tests; ServiceNow integration GA | Foundation + start of Discovery alpha |
